...
def get_answer(llm, messages):
    ...
    return answer.content, cb.total_cost, messages  # 変更: messagesを返す
...
def main():
        ...
        if not is_valid_url:
            ...
            if content:
                ...
                with st.spinner("ChatGPT is typing ..."):
                    answer, cost, sent_messages = get_answer(llm, st.session_state.messages)  # 変更: sent_messagesを取得
                ...
                for message in sent_messages:  # 変更: 送信されたメッセージを表示
                    st.write(f"**{message.type.capitalize()} Message:** {message.content}")
    ...
    total_cost_jpy = sum(costs) * EXCHANGE_RATE  # 変更: EXCHANGE_RATEを使用
    st.sidebar.markdown(f"**総費用: ¥{total_cost_jpy:.0f}**")
    # サイドバーに総コストを表示

    for cost in costs:
        st.sidebar.markdown(f"- ${cost:.5f} (¥{cost * EXCHANGE_RATE:.0f})")  # 変更: EXCHANGE_RATEを使用
        # 各リクエストのコストをサイドバーに表示
